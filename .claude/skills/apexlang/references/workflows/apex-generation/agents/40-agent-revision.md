...
- Deterministic button icon-position autofix: when a `@/text-with-icon` button has `icon` but no icon-position template option, add `t-Button--iconLeft`; when it has both `t-Button--iconLeft` and `t-Button--iconRight`, keep the explicitly requested side if known, otherwise keep left. Remove left/right icon-position options from `@/icon` buttons.
...
- Deterministic master-detail Content Row autofix:
  ...
  - For `INTERACTIVE_REPORT_LINK_COLUMN_TYPE_FORBIDDEN_001`, change only top-level Interactive Report column `type: link` to `type: plainText`; preserve the `link {}` block, `target`, `linkText`, heading, layout, source, comments, and every other column setting.
