...
1. ✅ **Показати дані в APEX** — ЗРОБЛЕНО (2026-07-19). app 200 має 3 браузери живих даних: Організації (158), Контрагенти (10576), Користувачі (490). Демо «структура + дані + UI» замкнуто.
   - *Follow-up*: переекспорт app 200 (`apex export -applicationid 200 -expType APEXLANG`) для звірки внутрішніх id стор. 9/10 у репо — блокує `ORA-06598` під SYS; export треба гнати під схемою **BAS_REVERSE** (її пароль не в доступних кредах). Репо лишається валідним джерелом: файли імпортуються один-в-один. Пастка синтаксису: у цій збірці SQLcl прапорці — `apex validate/import -workspaceId <id>` (не `-dir`), `apex export ... -expType APEXLANG` (не `-as-apexlang`); `-applicationid` лише для export.
...
**Рекомендація**: далі (2) — генератор регістрів відомостей (найбільший невзятий обсяг структури, 99 обʼєктів), потім (3) догрузка решти НСІ тим самим конвеєром. (4) ревю §9 — коли потрібні архітектурні рішення для чистового дизайну.
