...
- `015-bp-full-archive` — **повний архів 5 великих BusinessProcess** (замість
  семплу 500 з 012). Партиційний завантажувач за префіксом `Number` (пакет
  `RSD_BP_ARCHIVE` + `RSD_LOAD_LOG`, резюмовний, detached): `startswith(Number,
  'ДО-'||lpad(k,39,'0'))` = чанк ~1000 одним запитом, стоп за 5 порожніми
  (gap-safe). **365 302 header (== OData $count) + 3.37M ТЧ**, 0 INVALID. Проекція
  — reused 012 `project.sql`. **Пастка 12ГБ (ORA-12954)**: RAW.doc LOB ~9.3ГБ →
  проекція великих, reclaim `TRUNCATE ... DROP ALL STORAGE` (10741→1368МБ; shrink
  не бере — LOB retention), тоді решта. §9: поліморфні ТЧ (`ISPOLNITEL_REF`…) —
  NULL (проекція не резолвить; повна ре-проекція скинула резолв 500-семплу app-201).
...
- Живі дані: **23 довідники** (6 ядрових ~18k + 17 решти НСІ 1642) + **8 процесів** (3 малих повністю + **5 великих ПОВНІСТЮ — 365 302 header + 3.37M ТЧ** (батч 015; Согласование 185k / КомплексныйПроцесс 108k / Ознакомление 50k / Исполнение 12k / Утверждение 9.5k, кожен == OData $count)). RAW truncated (staging звільнено; стенд 1.4ГБ/12ГБ).
...
### ▶ Старт завтра — відкриті задачі (пріоритет згори)

1. **§9 поліморфні ТЧ повного архіву** — `ISPOLNITEL_REF_TYPE/ID` (і Предмет/…) = NULL
   після проекції 015 → у app 201 «Тип/Виконавець» порожні. Відновити: re-fetch полів
   `Исполнитель`+`_Type` Согласования (loader, ~1ГБ тимч. RAW) + resolve-UPDATE (шаблон —
   стор.12 app-200/app-201 views), потім `truncate` RAW. Той самий клас, що §9 4b. ~30 хв.
2. **BP Шар 2 — тиражування workflow** на Утверждение/Рассмотрение/Ознакомление/
   Регистрація **канонічним Switch** (скіл `apex-workflow`, не лінійний fallback). Пілот
   «Согласование» готовий (`013`, `export/`, `a81fce8`). Опц.: редагований IG погоджувачів.
3. **Blueprint-track на регістр або чистий документ** — розширити 014 на інший тип
   (регістр → зріз-view; документ → master-detail). Опц.: довести до **app 122** як цільового.
4. **ERP-контур `/Riverside`** — новий кластер з іншого джерела OData (BAS ERP, 41
   підсистема, `../BAS new/reference-erp/`): свій extract→spec→derive→install.
5. **Дрібне**: УведомленияПрограммы (6478, без Code — Code-alt пагінація); ~30 не-ядрових
   НСІ поза OData; ТЧ решти документів/процесів; §9 4b/6/7 при міграції потрібних каталогів.

**Зроблено 2026-07-19:** 014 blueprint-track → app 201 «ПОГОДЖЕННЯ» · 015 повний архів
5 великих BP (365 302 header + 3.37M ТЧ) · §9 4b проаналізовано+відкладено · 013 workflow-
пілот «Согласование» зібрано+протестовано · 016 ACL-фікс + APEX-скіли. Стенд 1.4ГБ/12ГБ, 0 INVALID.

---

**Деталі по пунктах (статуси):**

1. ✅ **Повний архів великих процесів — ЗРОБЛЕНО** (батч 015): усі 5 великих BP
   повністю (365 302 header == OData $count + 3.37M ТЧ), Number-prefix партиції.
   Залишок §9: резолв поліморфних ТЧ повного архіву (`ISPOLNITEL_REF`… — NULL після
   проекції; потребує re-fetch + resolve-UPDATE, RAW truncated). Кандидат разом з 4b.
