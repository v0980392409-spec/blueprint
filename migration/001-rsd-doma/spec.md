...
5. **EXTERNAL-цілі FK** (констрейнти відкладено): Catalog.БанковскиеСчета,
   ...
   обовʼязковий (NOT NULL FK-колонка)** — ✅ закрито батчем 002: FK
   `ENTITY_KIND_ID → RSD_ENUMS(ID)` (enum_type='ЮрФизЛицо'), LOV фільтрує тип.
   Решта цілей лишаються EXTERNAL до своїх батчів. `Enum.ТипыКонтактнойИнформації`
   — системне перечислення БСП, у дампі окремим обʼєктом відсутнє.
